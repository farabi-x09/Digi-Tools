    ...
    return (
        <div>
            <div key={item.id} className='space-y-5 p-5 rounded-2xl border-2 border-gray-300 '>
                ...
                <div>
                    <button onClick={hendleBuy}  disabled={buy} className=' btn bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white px-12 py-2 rounded-full'>{buy ? "Purchased" : "Buy Now"}</button>
                </div>
            </div>
        </div>
    );
